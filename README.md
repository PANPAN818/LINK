...
- 单角色聊天支持“情侣守护”空间：在明确共享授权后，按角色设定、聊天记忆和时间上下文调用 API 生成陪伴地图、路线停留、到达提醒、手机状态报告、生活印记与心愿便利贴；内容是角色互动模拟，不读取真实设备定位或系统解锁数据。
...
- Themes 的“全局”分栏支持整体缩放、持久化全屏开关和全站 CSS 主题预设；全站样式覆盖所有子页面、表单、卡片、导航、弹窗与浮层，且与线上/线下样式一致支持代码编辑、PNG 导入导出和分享。
...
- 角色消息通知完整展示本轮全部消息；Android App 使用单侧角色头像显示社交通知，并可把聊天、VOOM、全站/线上/线下主题和主页主题图片直接保存到系统相册；iOS App 通过系统分享面板导出图片。
