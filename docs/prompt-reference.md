...
角色设定、用户设定以及双方 LINK 昵称/签名中的 `{{char}}`、`<char>` 会在进入单聊提示词前替换为当前角色真名，`{{user}}`、`<user>` 会替换为当前绑定用户真名。
...
角色设定、用户设定、一对一会话总结、记忆手册、近期私聊/线下对话和世界书中的 `{{char}}`、`<char>` 会在每位角色的专属上下文中替换为该角色真名，`{{user}}`、`<user>` 会替换为当前用户真名。同一群中的不同角色会分别解析，不会共用某一个角色名。
