...
Themes 全局分栏的全屏开关在 Android 1.0.3 起由 `LinkDisplay` 原生插件持久化，并在 Activity 创建、恢复和重新获得焦点时同时使用全屏 Window flag 与 WindowInsetsController 隐藏顶部状态栏。旧 APK 只能使用 Capacitor SystemBars 回退，部分 Android ROM 可能会自行恢复状态栏。
