...
Themes 全局分栏的全屏开关在 Android 1.0.4 起由 `LinkDisplay` 原生插件持久化，并在 Activity 创建、恢复和重新获得焦点时同时使用全屏 Window flag、沉浸式兼容 flags 与 WindowInsetsController 隐藏顶部状态栏和底部导航/手势栏。旧 APK 只能使用 Capacitor SystemBars 回退，部分 Android ROM 可能会自行恢复系统栏。
